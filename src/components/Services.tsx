...
const Services = () => {
  ...
  const services = [
    ...
    },
    {
      ...
      gradient: "bg-gradient-to-br from-amber-500/20 to-yellow-400/20",
    },
    {
      img: hotel,
  ...
  return (
    <section id="services" className="section-padding bg-gradient-to-br from-emerald-50 via-white to-green-50/30 relative overflow-hidden mt-[-50px]">
      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className={`text-center mb-16 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-emerald-500/10 to-amber-500/10 border border-emerald-200/50 text-emerald-600 px-6 py-3 rounded-full font-medium mb-6 backdrop-blur-sm">
          ...
          <p className={`text-xl text-muted-foreground max-w-3xl mx-auto ${isRTL ? "font-arabic" : ""} leading-relaxed bg-white/60 backdrop-blur-sm rounded-2xl p-6 border border-white/20`}>
            {t("services.description")}
          </p>
        </div>

        {/* Services Grid */}
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 transform transition-all duration-1000 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          {services.map((service, index) => (
            <Link to={service.href} key={index} className="group relative block">
              {/* Background Glow Effect */}
              <div className={`absolute inset-0 ${service.gradient} rounded-3xl blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
              
              <Card className="relative group-hover:shadow-2xl group-hover:shadow-emerald-500/20 transition-all duration-500 ease-out transform group-hover:-translate-y-3 border-white/30 bg-white/70 backdrop-blur-md overflow-hidden">
                {/* Animated Border */}
                <div className={`absolute inset-0 bg-gradient-to-r ${service.color} rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -m-0.5`}></div>
                ...
                <CardHeader className="relative z-10 text-center pb-6 p-8">
                  ...
                  <div className="relative mb-6 transform group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500">
                    <div className={`absolute inset-0 bg-gradient-to-br ${service.color} rounded-2xl blur-md opacity-30 group-hover:opacity-50 transition-opacity duration-500`}></div>
                    <div className={`relative bg-gradient-to-br ${service.color} p-4 rounded-2xl shadow-lg`}>
                      <img 
                        src={service.img} 
                        alt={service.title}
                  ...
                  <CardTitle className={`text-xl font-bold flex items-center justify-center gap-3 ${isRTL ? "font-arabic" : ""}`}>
                    <span className="bg-gradient-to-r from-emerald-800 to-green-700 bg-clip-text text-transparent group-hover:from-amber-700 group-hover:to-yellow-600 transition-all duration-500">
                      {service.title}
                    </span>
                    {isRTL ? (
                      <ArrowLeftCircle className={`w-6 h-6 ${service.iconColor} transform group-hover:translate-x-1 group-hover:scale-110 transition-transform duration-300`} />
                    ) : (
                      <ArrowRight className={`w-6 h-6 ${service.iconColor} transform group-hover:translate-x-1 group-hover:scale-110 transition-transform duration-300`} />
                    )}
                  </CardTitle>

                  {/* Hover Effect Line */}
                  <div className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 w-0 h-1 bg-gradient-to-r ${service.color} rounded-full group-hover:w-3/4 transition-all duration-500 ease-out`}></div>
                </CardHeader>
              </Card>
            </Link>
          ))}
        ...
        <div className={`text-center mt-16 transform transition-all duration-1000 delay-600 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="relative">
            ...
            <div className="bg-gradient-to-r from-emerald-600 via-green-600 to-amber-600 rounded-3xl p-8 text-white shadow-2xl shadow-emerald-500/30 max-w-2xl mx-auto relative overflow-hidden">
              {/* Animated Background */}
              ...
              <div className="relative z-10">
                <h3 className={`text-2xl lg:text-3xl font-bold mb-4 ${isRTL ? "font-arabic" : ""} drop-shadow-sm`}>
