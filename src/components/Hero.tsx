import { ArrowLeft, ArrowRight, Shield, Clock, Award, Sparkles, Star } from "lucide-react";
    ...
    <section id="home"  className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-[25px] sm:px-6 py-20">
        <div className={`max-w-4xl transform transition-all duration-1000 ease-out ${
          ...
          <div className="mb-8">
            <div className={`inline-flex items-center gap-2 bg-gradient-to-r from-amber-500/20 to-emerald-600/20 border border-amber-400/30 text-amber-300 px-6 py-3 rounded-full ${
              isRTL ? 'font-arabic' : ''
          ...
          <div className="flex flex-col sm:flex-row gap-6 mb-12">
            ...
            <Button 
              variant="outline" 
              size="lg" 
              className={`group relative overflow-hidden text-white border-amber-400/50 hover:border-amber-400 hover:bg-amber-400/10 bg-amber-400/10 text-xl px-8 py-6 rounded-2xl backdrop-blur-sm ${
                isRTL ? 'font-arabic' : ''
              } hover:scale-105 transform transition-all duration-300 ease-out`}
            >
              <span className="relative z-10">{t('hero.free_consultation')}</span>
          ...
          <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 transform transition-all duration-1000 delay-500 ${
            isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
          }`}>
            {[
              { icon: Award, value: t('hero.stats.clients'), label: t('hero.stats.clients_label'), delay: 0 },
              { icon: Shield, value: t('hero.stats.success'), label: t('hero.stats.success_label'), delay: 200 },
              { icon: Clock, value: t('hero.stats.processing'), label: t('hero.stats.processing_label'), delay: 400 },
            ].map((stat, index) => (
              <div 
                key={index} 
