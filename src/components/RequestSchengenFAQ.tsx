...
function RequestSchengenFAQ() {
  ...
  return (
    <div className={` mx-auto p-6 lg:p-10 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-20 -right-20 w-60 h-60 bg-green-400/10 rounded-full blur-3xl"></div>
        <div className="absolute -bottom-20 -left-20 w-60 h-60 bg-amber-400/10 rounded-full blur-3xl"></div>
      </div>

      {/* Header Section */}
      <div className="text-center mb-12">
        <div className="inline-flex items-center gap-3 bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-200/50 text-green-600 px-6 py-3 rounded-full font-arabic font-medium mb-6 backdrop-blur-sm">
          <HelpCircle className="w-5 h-5" />
          {t("request.faqTitle")}
          <MessageCircle className="w-5 h-5" />
        </div>
        <h2 className="text-4xl lg:text-5xl font-bold  mb-4 font-arabic bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">
      ...
      <div className="relative">
        ...
        <div className="max-w-4xl m-auto bg-gradient-to-br from-white/80 via-white/60 to-emerald-50/30 backdrop-blur-md rounded-3xl p-8 shadow-2xl shadow-green-500/10 border border-white/50 relative overflow-hidden">
          ...
          {/* Footer CTA */}
          <div className="text-center mt-8 pt-8 border-t border-green-200/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
        {[
          ...
          <div key={index} className="group text-center">
            <div className="bg-gradient-to-br from-white/80 to-gray-50/80 backdrop-blur-sm rounded-2xl p-6 border border-green-200/30 hover:border-green-300/50 hover:shadow-lg hover:shadow-green-500/10 transition-all duration-500 ease-out transform group-hover:-translate-y-1">
              <div className="relative inline-block mb-4">
                <div className={`absolute inset-0 bg-gradient-to-br ${item.color} rounded-2xl blur-md opacity-0 group-hover:opacity-50 transition-opacity duration-500`}></div>
                <div className={`relative bg-gradient-to-br ${item.color} p-4 rounded-2xl shadow-lg`}>
                  <item.icon className="w-8 h-8 text-white" />
                </div>
              </div>
              <h4 className="font-semibold text-gray-800 font-arabic mb-2">{item.title}</h4>
              <p className="text-gray-600 font-arabic text-sm">{item.description}</p>
            </div>
          </div>
        ))}
      </div>
